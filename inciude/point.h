...
struct Point {
	...
	Point() : x(0), y(0) {} /*конструктор по умолчанию, устанавливающий начальные значения координат x и y равные 0.*/
	Point(T x, T y) : x(x), y(y) {} /*конструктор с параметрами для установки конкретных значений координат x и y.*/

	Point(const Point<T>& point) : x(point.x), y(point.y) {}

	double distance(const Point<T>& point) { // метод для вычисления расстояния между двумя точками.
		double a = point.x - x;
		double b = point.y - y;
